    ...
    <div className="grid min-h-screen lg:grid-cols-[.9fr_1.1fr]">
      <section className="relative hidden overflow-hidden bg-[#102a43] p-12 text-white lg:flex lg:flex-col">
        ...
        <div className="relative z-10 my-auto max-w-lg">
          <span className="inline-flex items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-400/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-emerald-300">
            <Sparkles className="size-3.5" /> Learning without borders
          </span>
          <h1 className="mt-6 text-5xl font-bold leading-[1.08] tracking-tight">
            Build skills.
            <br />
            Earn recognition.
            <br />
            <span className="text-emerald-400">Grow anywhere.</span>
          </h1>
          <p className="mt-6 max-w-md text-sm leading-7 text-slate-300">
            Join a global community learning practical skills through expert-led
            courses and verifiable certificates.
          </p>
          <div className="mt-9 grid grid-cols-3 gap-5 border-t border-white/10 pt-7">
            <AuthStat value="12k+" label="Learners" />
            <AuthStat value="80+" label="Countries" />
            <AuthStat value="4.9" label="Avg. rating" />
          </div>
        </div>
        <div className="relative z-10 flex items-center gap-2 text-[10px] text-slate-400">
          <ShieldCheck className="size-4 text-emerald-400" /> Secure account
          access · UI demonstration
        </div>
      </section>
      ...
      <section className="flex min-h-screen items-center justify-center px-5 py-10 sm:px-10">
        <div className="w-full max-w-md">
          <div className="mb-8 flex items-center justify-between">
            <Link
              href="/"
              className="inline-flex items-center gap-2 text-xs font-medium text-slate-500 hover:text-emerald-700"
            >
          ...
          {submitted ? (
            <SuccessState isLogin={isLogin} />
          ) : (
            <Card className="border-0 py-0 shadow-xl shadow-slate-200/60">
              <CardContent className="p-6 sm:p-8">
                <div>
                  ...
                  <p className="mt-2 text-xs leading-5 text-slate-500">
                    {isLogin
                      ...
                      : "Access free training and upgrade whenever you’re ready."}
                  </p>
                </div>
                <div className="mt-6 grid grid-cols-2 gap-2">
                  ...
                  >
                    <span className="font-bold text-sky-600">G</span> Google
                  </Button>
                  <Button
                    variant="outline"
                    type="button"
                    onClick={() => setSubmitted(true)}
                  >
                    <span className="font-bold text-slate-900">in</span>{" "}
                    LinkedIn
                  </Button>
                </div>
                ...
                <p className="mt-6 text-center text-xs text-slate-500">
                  {isLogin ? "New to CertiLearn?" : "Already have an account?"}{" "}
                  <Link
          ...
          <div className="mt-6 flex justify-center gap-5 text-[9px] text-slate-400">
            <span className="flex items-center gap-1">
              <LockKeyhole className="size-3" /> Secure sign-in
            </span>
            <span className="flex items-center gap-1">
              <Globe2 className="size-3" /> Global access
            </span>
          </div>
        </div>
      </section>
...
function FormField({
  ...
  return (
    <label className="block">
      <span className="mb-1.5 flex items-center justify-between text-[11px] font-semibold text-slate-700">
        {label}
        {trailing}
      </span>
      {children}
...
function SuccessState({ isLogin }: { isLogin: boolean }) {
  return (
    <Card className="border-0 py-0 text-center shadow-xl shadow-slate-200/60">
      <CardContent className="p-8">
        ...
        <p className="mx-auto mt-2 max-w-sm text-xs leading-6 text-slate-500">
          {isLogin
            ? "Your demo sign-in was successful. Continue to your learning dashboard."
